...
public class MockExternalController {
    ...
    /**
     * Mock CRM-системы (внешняя корпоративная информационная система).
     * Реальный вызов осуществляется через JCA-адаптер в enrollment-worker.
     */
    @PostMapping("/crm/notify")
    public ResponseEntity<Map<String, String>> crmNotify(@RequestBody Map<String, Object> request) {
        String operation = String.valueOf(request.get("operation"));
        String email = String.valueOf(request.get("userEmail"));
        String courseId = String.valueOf(request.get("courseId"));
        log.info("Mock CRM: received notification operation={}, email={}, course={}", operation, email, courseId);
        return ResponseEntity.ok(Map.of("status", "ACCEPTED", "operation", operation));
    }
}